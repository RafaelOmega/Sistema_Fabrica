...
class SaidaFilterProxyModel(QSortFilterProxyModel):
    """Proxy customizado para pesquisa de saídas.
    Filtra por sequência e data, case-insensitive."""
    ...
    def filterAcceptsRow(self, source_row, source_parent):
        ...
        saida = model.obter_saida(source_row)
        if saida is None:
            return False

        conteudo = " ".join(
            [
                "" if saida.get("sequencia") is None
                else str(saida["sequencia"]),
                "" if saida.get("data_saida") is None
                else saida["data_saida"].strftime("%d/%m/%Y"),
            ]
        ).lower()
